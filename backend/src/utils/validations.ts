...
import { getPassword } from "./makePassword"
...
export const createUserValidation = (user: string, users?: User[]) => {
    if (users) {
...
export const passwordAuthValid = (password: string, user: User | undefined) => {
    if (!Boolean(password)) {
        return { isValid: false, code: 404, title: "Data error", message: 'Password is invalid or not found' }
    }
    ...
    if (password !== getPassword(user?.password || '')) {
        return { isValid: false, code: 500, title: "Password is invalid", message: 'You entered the wrong password' }
    }

    return { isValid: true, code: 200, title: "Success", message: "Password is valid" }
}

export const userAuthValid = (user: string, users: User[]) => {
    if (!Boolean(user)) {
        return { isValid: false, code: 404, title: "Data error", message: 'Login is invalid or not found' }
    }
    ...
    if (users.find(({ login }) => login.toLowerCase() === user.toLowerCase())) {
        ...
        return { isValid: false, code: 404, title: "Login is invalid", message: 'Login is not found' }
    }
}